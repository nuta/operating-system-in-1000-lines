...
<Info>
本章は<EnagaBook chapter="9" title="システムコール" />の内容に対応しています。
</Info>
...
次にシステムコールを実際に呼び出す関数です。大体は [SBIの呼び出し](/book/kernel/hello-world#%E5%88%9D%E3%82%81%E3%81%A6%E3%81%AEsbi) の実装と同じです。
...
加えて、`putchar`システムコールを呼び出す `putchar` 関数を加えています。第1引数として文字を渡します。第2引数以降は、現在は未使用なので0を渡すことにします。

最後に、`user.h`に宣言を追加しておきます。
