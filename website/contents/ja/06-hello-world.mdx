...
ちなみに、callee (呼び出し先) の反対派はcaller (呼び出し元) です。
</Info>

各ローカル変数の宣言に使われている`register`と`__asm__("レジスタ名")`は、コンパイラに指定したレジスタに値を入れるよう指示するものです。本来であればインラインアセンブラで指定できればいいものなのですが、C言語 (正確にはGCC/clangの独自拡張) ではできないためこのトリックを使うことが多いです。
...
TODO: OpenSBIはなにしてる？シリアルポートドライバ
