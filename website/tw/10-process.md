...
「行程（Process）」是一個應用程式的實例。每個行程都有自己獨立的執行上下文與資源，例如虛擬位址空間。
...
接下來，我們來實作建立行程的初始化函式 `create_process`。它會接收一個進入點（entry point）作為參數，並回傳一個指向建立後的 `process` 結構體的指標：
...
我們已經實作了行程最基本的功能 ― 讓多個行程可以同時執行（並行）。現在我們來建立兩個行程：
...
`proc_a_entry` 和 `proc_b_entry` 函式分別是 A 行程與 B 行程的進入點。它們會透過 `putchar` 函式輸出一個字元後，利用 `switch_context` 函式切換到另一個行程。
...
在前一個實驗中，我們是直接呼叫 `switch_context` 函式，來指定「下一個要執行的行程」。但當行程數量變多時，這種做法會變得複雜。為了解決這個問題，我們將實作一個「排程器（scheduler）」，這是一個由核心負責的程式，用來決定下一個要執行的行程。

以下的 `yield` 函式就是排程器的實作：

> [!TIP]
>
> 「yield」這個詞，常被用來表示一個 API，它允許目前的行程主動讓出 CPU 給其他行程使用。
...
在這裡，我們會引入兩個全域變數：`current_proc` 指向目前正在執行的行程；而 `idle_proc` 則代表「當沒有任何行程可以執行時，要執行的閒置行程」。`idle_proc` 會在開機時建立，並作為行程 ID 為 `0` 的特殊行程，如下所示：
...
這段初始化的關鍵在於 `current_proc = idle_proc`。這樣可以確保開機階段的執行上下文被保存並作為 idle 行程的上下文還原。第一次呼叫 `yield` 函式時，會從 idle 行程切換到 A 行程；而之後再切換回 idle 時，會如同從當初那個 `yield` 呼叫中返回一樣。
...
在例外處理器（exception handler）中，我們會將執行狀態儲存到堆疊中。不過，現在我們為每個行程使用獨立的核心堆疊，因此這邊需要稍作調整。

首先，在行程切換時，將目前執行中行程的核心堆疊底部指標儲存到 `sscratch` 暫存器中。
...
因此，`sp` 現在指向的是目前執行中的行程的「核心（而非使用者）」堆疊。而 `sscratch` 則保存了例外發生當下原本的 `sp` 值（也就是使用者堆疊指標）。
...
重點是：每個行程都有自己獨立的核心堆疊。透過在上下文切換時交換 `sscratch` 的內容，我們就能在行程被中斷後再恢復執行，就像什麼事都沒發生一樣。

> [!TIP]
>
> 我們目前已經完成的是針對「核心」堆疊的上下文切換機制。應用程式使用的堆疊（即所謂的「使用者堆疊」）會與核心堆疊分開配置，這部分會在後續章節中實作。
...
如果你在沒有套用本章修改（也就是沒有從 `sscratch` 還原核心堆疊）的情況下執行這段程式，核心會直接卡住，什麼都不會顯示，並且你會在 QEMU 的記錄中看到以下輸出：
...
現在我們已經成功讓多個行程能夠同時執行，實現了一個具備「多工處理」能力的作業系統。
然而，目前的實作下，行程仍然可以任意讀寫核心的記憶體區域，這是非常危險的！
