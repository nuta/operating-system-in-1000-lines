...
1. `do-while` 敘述：  
  由於條件是 `while (0)`，這個區塊只會執行一次。這是 C 語言中定義多行巨集時的常見寫法。如果你只單純用 `{ ... }` 包起來，在某些情況下（例如搭配 `if` 使用）可能會造成語法邏輯錯誤。請參考[這篇](https://www.jpcert.or.jp/sc-rules/c-pre10-c.html)清楚的範例說明。另外注意，每一行結尾的 `\` 是為了讓 C 前置處理器把整個巨集當成單一行處理，讓換行不會被當成巨集的結尾。
