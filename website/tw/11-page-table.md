...
這種機制可以實作記憶體空間的隔離（虛擬位址空間隔離），達到核心與應用程式區域分離的效果，進而提升系統安全性。
...
最後，在 `create_process` 函式中將核心頁面加入對應的頁表。這些核心頁面從 `__kernel_base` 到 `__free_ram_end`，確保核心能夠存取靜態配置的區段（例如 `.text`），以及動態配置的記憶體區段（由 `alloc_pages` 管理）。
...
> [!TIP]
>
> 當核心啟動時，預設是未啟用分頁功能（`satp` 暫存器尚未設定），此時虛擬位址的行為就像是與實體位址完全相同。
...
輸出結果與前一章（上下文切換）完全相同。即使啟用了分頁功能，表面上也沒有任何變化。為了確認我們是否正確地設定了分頁表，我們來使用 QEMU monitor 檢查吧！

## 檢查分頁表內容

我們來看看 `0x80200000` 附近的虛擬位址是如何被對應的。如果設定正確，這些位址應該會對應到相同的實體位址，也就是 `(虛擬位址) == (實體位址)`。
...
> 這在檢查使用者空間（應用程式）的記憶體時特別有用，因為使用者空間的虛擬位址不等於實體位址，這點與我們目前的核心空間不同。

根據規範，第二層分頁表位於：`(0x20095001 >> 10) * 4096 = 0x80254000`。我們再次查看第 512 個項目，因為 `VPN[0] = (0x80200000 >> 12) & 0x3ff = 512`。
```
(qemu) xp /x 0x80254000+512*4
0000000080254800: 0x200800cf
```

數值 `0x200800cf` 對應的實體頁號為 `0x200800cf >> 10 = 0x80200`（根據規範，我們會忽略最低的 10 個位元，因為那是用來表示權限旗標的）。這代表虛擬位址 `0x80200000` 已成功對應到實體位址 `0x80200000`，正是我們想要的結果！我們也來轉儲（dump）整個第二層分頁表（共 1024 個項目）：
...
> [!TIP]
>
> 對初學者來說，除錯分頁表可能會相當困難。如果你的系統沒有如預期運作，可以參考「附錄：分頁機制除錯指南」章節來排查問題。
...
總結來說，你可以透過檢查 QEMU 的記錄、暫存器傾印與記憶體內容來找出問題所在。但最重要的一點是：_「仔細閱讀規範」_，因為忽略細節或誤解規範是非常常見的錯誤來源。
