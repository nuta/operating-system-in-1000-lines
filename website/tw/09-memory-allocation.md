...
透過在 linker script 中定義這些區域，而不是在程式中寫死位址（hardcode），可以讓 linker 自動決定這些區域的位置，以避免與核心的靜態資料區域重疊。
...
讓我們來實作一個可以動態配置記憶體的函式。我們不會像 `malloc` 那樣以「位元組（byte）」為單位來配置，而是以較大的單位，稱為「頁面（page）」來配置。一頁的大小通常是 4KB（4096 位元組）。

> [!TIP]
>
> 4KB = 4096 = 0x1000（十六進位表示）。因此，頁面對齊的位址在十六進位表示下會呈現出整齊的對齊效果。

以下的 `alloc_pages` 函式會動態配置 `n` 頁的記憶體，並回傳起始位址：
...
- `next_paddr` 指向「下一塊待配置區域」的起始位址（即尚未使用的區域）。每次配置記憶體時，`next_paddr` 都會依照配置的大小向後推進。
- `next_paddr` 的初始值是 `__free_ram` 的位址。這代表記憶體會從 `__free_ram` 開始依序配置。
- 由於 linker script 中使用了 `ALIGN(4096)`，`__free_ram` 會被放在 4KB 對齊的邊界上。因此，`alloc_pages` 函式回傳的位址一定是 4KB 對齊的。
- 如果嘗試配置超過 `__free_ram_end` 的範圍，換句話說，如果記憶體用完了，系統就會觸發核心恐慌（kernel panic）。
- `memset` 函式會確保配置的記憶體區域始終填滿零值。這是為了避免未初始化的記憶體造成難以除錯的問題。

是不是很簡單呢？但這種記憶體配置演算法有個很大的問題：已配置的記憶體無法釋放！不過，對我們這個簡單的作業系統來說已經足夠了。

> [!TIP]
>
> 此演算法被稱為「Bump 配置器（Bump allocator）」或「線性配置器（Linear allocator）」，實際上在某些不需要釋放記憶體的情境中會被使用。它是一種只需幾行就能實作、速度非常快的記憶體配置方式。
>
> 當需要實作釋放機制時，通常會採用基於 bitmap 的演算法，或者稱為夥伴系統（Buddy System）的方法。

## 來試試看記憶體配置

現在來測試我們實作的記憶體配置函式。在 `kernel_main` 中加入以下程式碼來測試：
